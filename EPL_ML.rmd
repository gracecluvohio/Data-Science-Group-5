...
Rename these so all the teams match
...
ds <- data[testing.idx, -37] 
data_xtrain <- as.matrix(data[testing.idx, -37] )
data_xtrain <- matrix(as.numeric(data_xtrain), ncol = ncol(data_xtrain))

data_ytrain <- as.matrix(data[testing.idx, 37] )
data_ytrain <- matrix(as.numeric(data_ytrain), ncol = ncol(data_ytrain))

#Training
data_xval <- as.matrix(data[-testing.idx, -37] )
data_xval <- matrix(as.numeric(data_xval), ncol = ncol(data_xval))

data_yval <- as.matrix(data[-testing.idx, 37] )
data_yval <- matrix(as.numeric(data_yval), ncol = ncol(data_yval))

```

### Reserve 500 validation entries

Extract the first 500 elements from our Testing data for use of
prediction later on.

``` {r echo = T}
# Testing 
# validation.idx <- c(1:500)

# data_xval <- data_xtest[validation.idx, ]
# data_yval <- as.matrix(data_ytest[validation.idx])

# data_xtest <- data_xtest[-validation.idx, ]
# data_ytest <- as.matrix(data_ytest[-validation.idx])
```
...
model <- keras_model_sequential() %>%
  layer_dense(units = 30, activation = "relu", input_shape = c(p)) %>%  # 1 layer with 16 neurons. default activation is relu
  layer_dense(units = 9, activation = "relu") %>%  # layer 2 with 8 neurons
...
``` {r echo = T}
num_correct <- sum(vs.actual$correct)
percent <- num_correct/500
```
